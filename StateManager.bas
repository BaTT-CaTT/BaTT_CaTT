...
Sub GetSetting2(Key As String, DefaultValue As String)
	If setting.IsInitialized = False Then
		'load the stored settings
		If File.Exists(File.DirInternal, settingsFileName) Then 
			setting = File.ReadMap(File.DirInternal, settingsFileName)
		Else
			Return DefaultValue
		End If
	End If
	Dim v As String
	v = setting.GetDefault(Key.ToLowerCase, DefaultValue)
	Return v
End Sub
'Gets the setting value associated with the given key.
...
Sub SetSetting(Key As String, Value As String)
	If setting.IsInitialized = False Then
		...
		End If
	End If
	setting.Put(Key.ToLowerCase, Value)
End Sub
'Stored the settings in a file
Sub SaveSettings
	If setting.IsInitialized Then
		File.WriteMap(File.DirInternal, settingsFileName, setting)
	End If
End Sub
