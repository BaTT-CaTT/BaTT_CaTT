...
Public Sub Initialize(Module As Object, svList As ScrollView2D, subCheck As String, subClick As String, subLongClick As String, DividerHeight As Int)
	sv = svList
...
Private Sub CommonJump(NewPosition As Int)
	...
	Do Until PreviousPosition = sv.VerticalScrollPosition
		PreviousPosition = sv.VerticalScrollPosition
		sv.VerticalScrollPosition = NewPosition
		DoEvents
	Loop
End Sub
	...
	If AddedContent.IsInitialized Then
		...
		For i = 0 To sv.Panel.NumberOfViews - 1
			...
			Else If pnl.Tag = ItemID Then
				...
				If ContentBottom > sv.VerticalScrollPosition + getHeight Then
					' If needed, we scroll to display the added content
					Dim NewPosition As Int
					DoEvents
					NewPosition = sv.VerticalScrollPosition + Min(ContentBottom - sv.VerticalScrollPosition - getHeight, getHeight) + 15dip
...
Private Sub MoveDDPanel(Y As Int)
	...
	If DragAndDrop.Top < sv.VerticalScrollPosition Then sv.VerticalScrollPosition = DragAndDrop.Top
	If DragAndDrop.Top + DragAndDrop.Height > sv.VerticalScrollPosition + getHeight Then
		sv.VerticalScrollPosition = DragAndDrop.Top + DragAndDrop.Height - getHeight
	End If
End Sub
